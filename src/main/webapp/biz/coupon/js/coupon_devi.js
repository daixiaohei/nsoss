 ...
 * Date: 2014-09-24
 */
define('biz/coupon/js/coupon_devi',function(require,exports,module){
	...
	var searchUrl = baseUrl+'/coupon/queryCouponDevi.do';
	var saveUrl = baseUrl+'/coupon/saveCouponDevi.do';//add or edit
	var deleteUrl = baseUrl+'/coupon/deleteCouponDevi.do';
	var loadDeviUrl = baseUrl+'/coupon/loadDevi.do';
	
	//表头
	var columns = [
        {title : '优惠券名',dataIndex :'couponName', width:'20%',elCls:'center'},
		{title : '设备列表',dataIndex :'deviName', width:'20%',elCls:'center'}
    ];
	...
	//用户操作 (CRUD) 
    var Operation = {
    	Condition: {couponId:couponId,deviName:''},
    	data: {id:null,couponId:couponId,deviceListId:null},
    	add: function(){//增
    		$("#obj").hide();$("#optObj").show();$("#optTitle").html("新增绑定设备");
	    ...
	    edit: function(){//改
	    	...
	    	$("#deviName").val(selections[0].deviceListId).change();
	    ...
	    initView: function(){
	    	...
	    	J.GetJson(loadDeviUrl,{},function(data){
	    		var options = '';
	    		for(var i in data){
	    		   options += '<option value="'+data[i].id+'">'+data[i].name+'</option>';
	    		}
	    		$("#deviName").append(options);
	    	},function(){
	    		console.log('加载网点失败');
	    	});
	    }
    ...
    var eventMap = {
		...
		"#srh_name change" : function(){Operation.Condition.deviName = $(this).trim();},
		"#deviName change" : function(){Operation.data.deviceListId = $(this).trim();}
	};
