...
define('biz/roleMenu/js/roleMenu',function(require,exports,module){
	...
	var initMenuNameUrl = baseUrl+'/organize/findMenuName.do';
    ...
    var Operation = {
    	...
    	initMenu:function(){
    		$.get(initMenuNameUrl,function(ret){
    			$("#menuId").empty();
    			var options = '';
    			for(var d in ret){
    				options += '<option  value="'+ret[d].menuId+'">'+ret[d].menuName+'</option>';
    			}    			
        		$("#menuId").append(options);
	    ...
	    edit: function(){//改
	        ...
	        $("#roleId").val(selections[0].roleId).change();
	        $("#menuId").val(selections[0].menuId).change();
